...
static void partition_format(struct partition* part){
    ...
    uint32_t inode_bitmap_sects = DIV_ROUND_UP(MAX_FILES_PER_PART,BITS_PER_SECTOR);//一个inode对应一个bit的位图，这里一个扇区即可管理所有inode
    ...
    uint32_t block_bitmap_bit_len = free_sects - block_bitmap_sects;//块位图中位的长度
    ...
    while(channel_no < channel_cnt){
        ...
        while(dev_no<2){
            ...
            while(part_idx < 12){
                ...
                if(part->sec_cnt !=0){ //如果存在分区
                    memset(sb_buf,0,SECTOR_SIZE);
                    /*读取超级块，根据魔术判断是否存在超级块*/
                    ide_read(hd,part->start_lba+1,sb_buf,1);
                    /*只支持自己的文件系统，若磁盘上存在文件系统则不格式分区*/
                    if(sb_buf->magic == 0x1950318){
